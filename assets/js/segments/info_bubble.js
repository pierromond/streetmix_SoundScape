...
// Acoustics variables
// emission law coefficient (see CNOSSOS-EU)
var AR_b = [88.7, 91.5, 96.7, 97.4, 90.9, 83.8]
var BR_b = [35.8, 32.6, 23.8, 30.1, 36.2, 38.3]
var AP_b = [96.5, 98.8, 96.8, 98.6, 95.2, 88.8]
var BP_b = [4.7, 6.4, 6.5, 6.5, 6.5, 6.5]
var AR_c = [85.7, 84.5, 90.2, 97.3, 93.9, 84.1]
var BR_c = [41.5, 38.9, 25.7, 32.5, 37.2, 39.0]
var AP_c = [89.2, 88.0, 85.9, 84.2, 86.9, 83.3]
var BP_c = [0, 0, 2.6, 2.9, 1.5, 2.3]
// A Ponderation
var pondA = [-16, -8.5, -3, 0, 1, 1]
...
var _infoBubble = {
    ...
    updateDescriptionInContents: function (segment) {
        ...
        // Acoustics part ---------- (Maybe better put in a independant file)
        var width_=[];
        
        // If the mouse is over Sidewalk
        if (segmentInfo.name == 'Sidewalk') {
            var el = document.createElement('div')
            el.classList.add('description-prompt')
            var i = 1
            var LwA = []
            for (i = 0; i < street.segments.length; i++) {
                LwA[i] = 20;
                var speed = ''
                var flow = ''
                var segment_i = street.segments[i]
                var segmentInfo_i = SEGMENT_INFO[segment_i.type]
                var variantInfo_i = SEGMENT_INFO[segment_i.type].details[segment.variantString]
                width_[i] = segment_i.width
                if (segmentInfo_i.speed) {
                    var speed = segmentInfo_i.speed
                    var flow = segmentInfo_i.flow
                    var LRw = []
                    var LPw = []
                    var Lw = []
                    var Lweq = []

                    if (segmentInfo_i.name == 'Drive lane') {
                        var AR = AR_c
                        var AP = AP_c
                        var BR = BR_c
                        var BP = BP_c
                    } else if (segmentInfo_i.name == 'Bus lane') {
                        var AR = AR_b
                        var AP = AP_b
                        var BR = BR_b
                        var BP = BP_b
                    ...
                    for (f = 0; f < 6; f++) {
                        LRw[f] = AR[f] + BR[f] * Math.log10(speed / 70)
                        LPw[f] = AP[f] + BP[f] * ((speed - 70) / 70)
                        Lw[f] = 10 * Math.log10(Math.pow(10, LRw[f] / 10) + Math.pow(10, LPw[f] / 10))
                        Lweq[f] = Lw[f] + 10 * Math.log10(flow / 1000 * speed)
                    }
                    var total = 0
                    for (var ii = 0; ii < 6; ii++) {
                        total = total + Math.pow(10, (Lweq[ii] + pondA[ii]) / 10)
                    }
                    LwA[i] = 10 * Math.log10(total)
                }
            }
            LwA[pos_seg] = 40
            var pos_seg = segment.el.dataNo
            var dist = []
            dist[0] = 0.3048 * width_[0] / 2
            for (i = 1; i < street.segments.length; i++) {
                dist[i] = dist[i - 1] + 0.3048 * width_[i - 1] / 2 + 0.3048 * width_[i] / 2
            }
            ...
            for (i = 0; i < street.segments.length; i++) {
                ...
                if (dist[i] != dist[pos_seg]) {
                    if (LwA[i] > 0) {
                        Ls = LwA[i] - (20 * Math.log10(Math.abs(dist[i] - dist[pos_seg])) + 1)
                        LwA[pos_seg] = 10 * Math.log10(Math.pow(10, LwA[pos_seg] / 10) + Math.pow(10, (LwA[i] - (20 * Math.log10(Math.abs(dist[i] - dist[pos_seg])) + 1)) / 10))
                    }
                }
                if (segmentInfo_i.name == 'Drive lane') {
                    el.innerHTML += '<audio id="Seg' + i + '" src="/sounds/Street.wav" autoplay>    </audio> '
                    nom[k] = "Seg" + i
                    volu[k] = Ls
                    k = k + 1
                } else if (segmentInfo_i.name == 'Bus lane') {
                    ...
                    volu[k] = Ls
                    k = k + 1
                } else if (segmentInfo_i.name == 'Sidewalk with a tree') {
                    el.innerHTML += '<audio id="Seg' + i + '" src="/sounds/Birds.wav" autoplay>  </audio> '
                    nom[k] = "Seg" + i
                    volu[k] = 75 - (20 * Math.log10(Math.abs(dist[i] - dist[pos_seg])) + 1)
                    k = k + 1
                }

            }
            el.innerHTML += '<audio id="Voice" src="/sounds/Voice.wav" autoplay>  </audio> '
		
            el.innerHTML +=  Math.floor(LwA[pos_seg]) +' dBA'
            _infoBubble.el.appendChild(el)
            for (kk = 0; kk < k; kk++) {
                console.log(volu[kk])
                setvolume(volu[kk], nom[kk])
            }
            setvolume(80, "Voice")
        ...
        var speed = ''
        var flow = ''
        if (variantInfo && variantInfo.speed) {
            var speed = variantInfo.speed
            var flow = variantInfo.flow
        } else if (segmentInfo && segmentInfo.speed) {
            var speed = segmentInfo.speed
            var flow = segmentInfo.flow
        }
        if (speed) {
            var el = document.createElement('div');
            el.classList.add('description-prompt');
            ...
            var Lweq = []
            var LwA = []
            if (segmentInfo.name == 'Drive lane') {
                var AR = AR_c
                var AP = AP_c
                var BR = BR_c
                var BP = BP_c
            } else if (segmentInfo.name == 'Bus lane') {
                var AR = AR_b
                var AP = AP_b
                var BR = BR_b
                var BP = BP_b
            ...
            var total = 0
            for (var i = 0; i < 6; i++) {
                total = total + Math.pow(10, (Lweq[i] + pondA[i]) / 10)
            }
            LwA = 10 * Math.log10(total)
            console.log(LwA)
            el.innerHTML += 'Speed:' +speed+ ' km/h - Flow : ' + flow +' veh/h <br> Lw :' + Math.floor(LwA) +' dBA'
...
function setvolume (vol, id_) {
    document.getElementById(id_).volume = Math.pow(10, (vol - 100) / 20)
}